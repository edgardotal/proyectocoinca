...
@Component({
  selector: 'app-cava4',
  templateUrl: './cava4.component.html',
  styleUrl: './cava4.component.scss'
})
export class Cava4Component {
    ...
    this.cava.forEach(row => {
      row.horizontals.forEach(horizontal => {
        horizontal.verticals.forEach(vertical => {
          ...
          this.cavaPosicionesService.getEstadoPosicion(4, fila, nivel, posicion).subscribe(res => {
            res.forEach(posicionData => {
              // Solo actualiza el estado si cava_id, fila, nivel y posicion coinciden
              if (posicionData.cava_id === 4 && 
