...
@Component({
  selector: 'app-cava5',
  templateUrl: './cava5.component.html',
  styleUrl: './cava5.component.scss'
})
export class Cava5Component {
    ...
    this.cava.forEach(row => {
      row.horizontals.forEach(horizontal => {
        horizontal.verticals.forEach(vertical => {
          ...
          this.cavaPosicionesService.getEstadoPosicion(5, fila, nivel, posicion).subscribe(res => {
            res.forEach(posicionData => {
              // Solo actualiza el estado si cava_id, fila, nivel y posicion coinciden
              if (posicionData.cava_id === 5 && 
