...
@Component({
  selector: 'app-cava6',
  templateUrl: './cava6.component.html',
  styleUrl: './cava6.component.scss'
})
export class Cava6Component {
    ...
    this.cava.forEach(row => {
      row.horizontals.forEach(horizontal => {
        horizontal.verticals.forEach(vertical => {
          ...
          this.cavaPosicionesService.getEstadoPosicion(6, fila, nivel, posicion).subscribe(res => {
            res.forEach(posicionData => {
              // Solo actualiza el estado si cava_id, fila, nivel y posicion coinciden
              if (posicionData.cava_id === 6 && 
