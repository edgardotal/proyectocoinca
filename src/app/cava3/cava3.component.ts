import { Component, ElementRef, ViewChild, AfterViewInit, OnInit, Output, EventEmitter } from '@angular/core';
...
@Component({
  selector: 'app-cava3',
  templateUrl: './cava3.component.html',
  styleUrls: ['./cava3.component.scss'] // Corregido aquí
})
export class Cava3Component implements AfterViewInit, OnInit {
    ...
    this.cava.forEach(row => {
      row.horizontals.forEach(horizontal => {
        horizontal.verticals.forEach(vertical => {
          ...
          this.cavaPosicionesService.getEstadoPosicion(3, fila, nivel, posicion).subscribe(res => {
            res.forEach(posicionData => {
              // Solo actualiza el estado si cava_id, fila, nivel y posicion coinciden
              if (posicionData.cava_id === 3 && 
