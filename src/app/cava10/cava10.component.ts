...
@Component({
  selector: 'app-cava10',
  templateUrl: './cava10.component.html',
  styleUrl: './cava10.component.scss'
})
export class Cava10Component {
  ...
  cargarEstadosPosiciones() {
    this.cava.forEach(row => {
      row.horizontals.forEach(horizontal => {
        ...
        this.cavaPosicionesService.getEstadoPosicion(10, fila, 1, posicion).subscribe(res => {
          res.forEach(posicionData => {
            if (posicionData.cava_id === 10 && posicionData.fila === fila) {
