...
@Component({
  selector: 'app-cava2',
  templateUrl: './cava2.component.html',
  styleUrl: './cava2.component.scss'
})
export class Cava2Component {
    ...
    this.cava.forEach(row => {
      row.horizontals.forEach(horizontal => {
        horizontal.verticals.forEach(vertical => {
          ...
          this.cavaPosicionesService.getEstadoPosicion(2, fila, nivel, posicion).subscribe(res => {
            res.forEach(posicionData => {
              // Solo actualiza el estado si cava_id, fila, nivel y posicion coinciden
              if (posicionData.cava_id === 2 && 
