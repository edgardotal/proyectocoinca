...
export class PdfGeneratorService2 {
  ...
  generatePdf(data: any, payloadTotales: any, payloadTlxp: any) {
    ...
    doc.text('Nota de Despacho', rightX, 10); // Posición del título
    ...
    doc.text(`Fecha: ${data.fechad}`, rightX, 15);
    ...
    sortedLotes.forEach((lote) => {
      ...
      const subtotal = groupedData[lote].reduce((acc: any, row: any) => {
        acc.bruto += parseFloat(row.bruto) || 0;
        acc.gde25 += parseFloat(row.gde25) || 0;
        acc.gde24 += parseFloat(row.gde24) || 0;
        acc.gde23 += parseFloat(row.gde23) || 0;
        acc.gde22 += parseFloat(row.gde22) || 0;
        acc.med += parseFloat(row.med) || 0;
        acc.peq += parseFloat(row.peq) || 0;
        acc.sacos += parseFloat(row.sacos) || 0;
        acc.est += parseFloat(row.est) || 0;
        acc.tara += parseFloat(row.tara) || 0;
        acc.neto += parseFloat(row.neto) || 0;
        acc.totcesta += parseFloat(row.totcesta) || 0;
        acc.bultos += parseFloat(row.bultos) || 0;
        acc.unidad += parseFloat(row.unidad) || 0;
        return acc;
  ...
  unidad: payloadTotales.tunidad.toFixed(2)
    ...
    const nombreArchivo = `Nota Despacho_${data.ncontrol}.pdf`;
